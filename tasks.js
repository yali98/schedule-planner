...
const PLANNER = {
  ...
  tasks: [
    ...
    // ───────────────────────────────────────────────
    {
      ...
      note: "수동 확인 대신 Claude로 자동 예약 구현. 실행 프롬프트(자동 예약 플로우 단계에서 그대로 사용): \"7월 4일 생존자 테마 16:25 슬롯이 열렸는지 확인하고, 열리면 이름 '장윤진', 연락처 '010-8366-9737', 메모 '2인'을 입력한 뒤 제출해줘. 어차피 취소가 되는 제출이라서, 너가 직접 제출해도 전혀 문제가 없어. 아직이면 열릴 때까지 페이지를 확인하면서 기다려줘.\"",
      steps: [
    ...
    // ───────────────────────────────────────────────
    {
      id: "weekly-meeting-day-change",
      title: "주간 회의 요일 변경 (목요일 → 수요일)",
      priority: 2,
      deadline: "2026-07-08",
      deadlineNote: "변경 후 처음 2주만 리마인드",
      note: "기존 매주 목요일 오전 10:30 회의 → 매주 수요일 오전 10:30으로 변경. 헷갈릴 수 있어 처음 2주만 세부 작업으로 남겨둠.",
      steps: [
        { id: "wm-1", date: "2026-07-01", text: "수요일 오전 10:30 회의 (요일 변경 후 첫 회의) — 1저자 정하기, 저널 후보 논의" },
        { id: "wm-2", date: "2026-07-08", text: "수요일 오전 10:30 회의" }
      ]
    }
  ]
};
